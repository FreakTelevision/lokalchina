...
import { Globe, MapPin, Calendar, Mail, Stethoscope } from 'lucide-react';
        ...
        <nav className="hidden md:flex items-center gap-12 text-[11px] tracking-[0.25em] font-light uppercase text-gray-200">
          ...
          <div className="relative group cursor-pointer py-2">
            ...
            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-64 bg-[#0b0c10]/95 backdrop-blur-md border border-white/10 p-2 flex flex-col opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-300 z-50 shadow-2xl">
              ...
              <Link href={`/${locale}/services/medical-concierge`} className="text-[10px] tracking-widest uppercase font-medium text-white bg-white/5 px-4 py-3 border-t border-white/5">Medical Concierge</Link>
        ...
        <div className="flex items-center gap-6">
          <div className="relative group">
            ...
            <div className="absolute right-0 top-full mt-2 bg-black/90 backdrop-blur-md border border-white/10 rounded-sm py-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 min-w-[130px] z-50">
              {allLocales.map((l) => (<Link key={l.code} href={`/${l.code}/services/medical-concierge`} className={`block px-4 py-2 text-[11px] tracking-wider uppercase ${l.code === locale ? 'text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>{l.label}</Link>))}
            </div>
          </div>
      ...
      <section className="max-w-3xl mx-auto px-6 pb-32">
        <div className="bg-[#121318] border border-white/10 p-8 md:p-12 shadow-2xl">
          ...
          <form className="space-y-8">
            <div className="space-y-2"><label className="block text-[10px] tracking-[0.2em] uppercase text-gray-400 font-medium flex items-center gap-2"><Stethoscope className="w-3.5 h-3.5 text-white stroke-[1.5]" /> Medical Need / Speciality</label><input type="text" placeholder="e.g. cardiac surgery, orthopaedics, TCM wellness, dental implants..." className="w-full bg-black/40 border border-white/10 focus:border-white px-4 py-3.5 text-xs tracking-wider outline-none text-white font-light" required /></div>
