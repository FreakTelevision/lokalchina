...
import { Globe, MapPin, Calendar, Users, DollarSign, Mail } from 'lucide-react';
        ...
        <div className="flex items-center gap-6">
          <div className="relative group">
            ...
            <div className="absolute right-0 top-full mt-2 bg-black/90 backdrop-blur-md border border-white/10 rounded-sm py-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 min-w-[130px] z-50">
              {allLocales.map((l) => (
                <Link key={l.code} href={`/${l.code}/services/bespoke-travel`} className={`block px-4 py-2 text-[11px] tracking-wider uppercase ${l.code === locale ? 'text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>{l.label}</Link>
              ))}
            </div>
          </div>
        ...
        <div className="bg-[#121318] border border-white/10 p-8 md:p-12 shadow-2xl">
          ...
          <form className="space-y-8">
            <div className="space-y-2">
              ...
              <input type="text" placeholder="e.g. Shanxi temples, Jingdezhen ceramics, Yiwu sourcing..." className="w-full bg-black/40 border border-white/10 focus:border-white px-4 py-3.5 text-xs tracking-wider outline-none text-white font-light" required />
            </div>
            <div className="space-y-2">
              ...
              <input type="text" placeholder="e.g. 10 days, around October" className="w-full bg-black/40 border border-white/10 focus:border-white px-4 py-3.5 text-xs tracking-wider outline-none text-white font-light" required />
            </div>
            <div className="space-y-2">
