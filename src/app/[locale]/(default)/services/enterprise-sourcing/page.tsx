...
export default async function EnterpriseSourcingPage() {
  return (
    <div className={`bg-white text-black min-h-screen antialiased selection:bg-white selection:text-black ${barlow.variable} ${montserrat.variable}`}>
      <section className="relative min-h-[35vh] flex flex-col justify-center items-center text-center px-6 pt-12 pb-6">
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-[0.3em] uppercase text-white mb-4" style={{ fontFamily: 'var(--font-condensed)' }}>SUPPLIER SOURCING SOLUTIONS</h1>
      ...
      <section className="max-w-3xl mx-auto px-6 pb-32">
        <div className="bg-[#121318] border border-white/10 p-8 md:p-12 shadow-2xl">
          ...
          <form className="space-y-8">
            <div className="space-y-2"><label className="block text-[10px] tracking-[0.2em] uppercase text-gray-400 font-medium flex items-center gap-2"><Package className="w-3.5 h-3.5 text-white stroke-[1.5]" /> Product Categories</label><input type="text" placeholder="e.g. kitchenware, electronics, toys, textiles..." className="w-full bg-black/40 border border-white/10 focus:border-white px-4 py-3.5 text-xs tracking-wider outline-none text-white font-light" required /></div>
            ...
            <div className="space-y-2"><label className="block text-[10px] tracking-[0.2em] uppercase text-gray-400 font-medium flex items-center gap-2"><Calendar className="w-3.5 h-3.5 text-white stroke-[1.5]" /> Preferred Timing</label><input type="text" placeholder="e.g. March 2027, or during Yiwu Fair in October" className="w-full bg-black/40 border border-white/10 focus:border-white px-4 py-3.5 text-xs tracking-wider outline-none text-white font-light" required /></div>
